#!/bin/sh
##
##    Docker image for Penpot Exporter.
...
init() {
    ...
    if [ ! -f "./.docker-version" ]; then
        log "Penpot exporter init to $(cat /app/src/.docker-version)..."
    elif ! cmp --silent "./.docker-version" "/app/src/.docker-version"; then
        log "Penpot exporter update from $(cat ./.docker-version) to $(cat /app/src/.docker-version)..."
    fi

    if [ -n "${PENPOT_PUBLIC_URI}" ]; then
        log "Forcing update of Penpot frontend public URI..."
        sed -i -e "s|http://localhost:3449|${PENPOT_PUBLIC_URI}|g" ./app.js
    fi

    cp "/app/src/.docker-version" "./.docker-version"
}
